...
def sample_and_test(args):
    ...
    load_checkpoint(checkpoint_file, gen_diffusive_b,'gen_diffusive_b',epoch=str(epoch_chosen), device = device)
    ...
    PG_im1=np.zeros((256,256,len(data_loader)))
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        PG_im1[:,:,iteration]=np.squeeze(fake_sample1.cpu().numpy())

        #保存为nii
        import SimpleITK as  sitk
        a = fake_sample1.detach().cpu().numpy()
        nii = sitk.GetImageFromArray(a)
        torchvision.utils.sitk.WriteImage(nii, '{}/{}_samples1_{}.nii.gz'.format(save_dir, phase, iteration),normalize=True)
    ...
    f = h5py.File(save_dir + '/im_PG.mat',  "w")
    f.create_dataset('images_'+args.contrast1+'PG', data=PG_im1)
    f.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta2', type=float, default=0.9,
                            help='beta2 for adam')
    parser.add_argument('--contrast1', type=str, default='CBCT',
                        help='contrast selection for model')
    parser.add_argument('--contrast2', type=str, default='CT',
                        help='contrast selection for model')
